...
while True:
    ...
    for div_ss_book_box in div_ss_book_box_list:
        ...
        # href로 작성된 키를 전다하고 해당 value를 받아 변수에 저장.
        ...
        try:
            ...
            # 엑셀에 이미지 저장
            # worksheet.insert_image('배치할 셀번호', 이미지제목, {'image_data':바이트로 변환한 이밎, 기타 속성...})
            worksheet.insert_image(f'A{cnt}', img_url['src'], {'image_data':img_data, 'x_scale':0.5, 'y_scale':0.5})
        except:
            # 파이썬에서는 블록구조에 아무것도 쓰지 않으면 에러이다.
