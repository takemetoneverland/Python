...
# 뷰티풀수프 객체 생성
# 뷰티플수프 객체를 생성하면서, 셀레늄이 가지고 온 html 소스코드를
# 제공하고, 해당 소스코드를 html 문법으로 변환하라는 주문.
soup = BeautifulSoup(src, 'html.parser')

'''
- 뷰티풀수프를 사용해서 수정하고 싶은 데이터가 들어있는
 태그를 부분 추출할 수 있다.

- find_all() 메서드는 인수값으로 추출하고자 하는 태그의
 이름을 적으면 해당 태그만 전부 추출하여 리스트에 담아 대입한다.
'''
...
# print(first_book) -> li안에 필요한 텍스트가 다 있다. 2,3,4번째 li의 텍스트를 가져와야한다.

# text = 태그를 제외한 사용자가 실제로 브라우저에서 확인 가능한
